...
RSpec.describe "Public identifiers", type: :request do
  ...
  describe "to_param" do
    ...
    it "routes goals, funnels, links and memberships by UUID" do
      ...
      [goal, funnel, link, membership].each do |record|
  ...
  describe "generated links actually resolve" do
    ...
    it "follows every goal and funnel link generated on their index pages" do
      create(:goal, site: site)
      create(:funnel, site: site)

      %W[/sites/#{site.to_param}/goals /sites/#{site.to_param}/funnels].each do |index|
        get index
        expect(response).to have_http_status(:ok)

        response.body.scan(%r{href="(/sites/[^"]*/(?:goals|funnels)/[^"]+)"}).flatten.uniq.each do |href|
  ...
  describe "no integer identifiers are exposed" do
    it "emits no /sites/<integer> or /goals/<integer> anywhere on the dashboard" do
      ...
      offenders = hrefs.grep(%r{/(sites|goals|funnels|shared_links|members)/\d+(/|\z|\?)})
