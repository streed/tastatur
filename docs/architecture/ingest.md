...
**Every numeric segment collapses to `:id`, whatever its length** — `/player/51`
identifies a person as surely as `/orders/1048576` does, so the digit count is
not allowed to decide. The one exception is date-shaped: **a four-digit year in
1900–2100 is kept, and so are the month (1–12) and day (1–31) that immediately
follow it** (`/2026/07/15/roundup`). Date-organised URLs are extremely common
(`/2026/07/roundup`, `/blog/2025/hello`) and collapsing the year would destroy
the top-pages report for every publication that uses them. A short number that is
*not* in that date run — `/section/07/detail`, `/player/51` — is a record id or a
page number and collapses. That is why the scrub is position-aware rather than a
per-segment regex, and it is enforced by a spec, not by inspection. (The earlier
